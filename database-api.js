...
// This version works with the proxied API (through Node.js server)

class ProductDatabaseAPI {
    constructor() {
        this.apiUrl = '/api'; // Uses proxy through Node.js server
    ...
    async init() {
        try {
            ...
            if (response.ok) {
                ...
                console.log('Connected to API server through proxy');
            } else {
    ...
    fallbackToLocalStorage() {
        console.warn('Using localStorage fallback - start the API server for database sync');
    ...
    // Get all themes (brands) from database
    async getThemes() {
        try {
            const response = await fetch(`${this.apiUrl}/themes`);
