...
async def process_cryptobot_order_redirect(callback: types.CallbackQuery, code: str):
    ...
    if not invoice_data:
        ...
        await callback.message.answer(error_message, reply_markup=await main_menu_kb())
...
async def process_catalog_row_width_button(callback: types.CallbackQuery, state: FSMContext):
    ...
    # Получаем данные из состояния до выполнения других операций
    async with state.proxy() as data:
...
async def skip_catalog_row_width(callback: types.CallbackQuery, state: FSMContext):
    ...
    if item_type == 'product':
        ...
        if is_parent_grid_3x6:
            ...
            await CatalogAddStates.waiting_for_price.set()
    elif item_type == 'category' or item_type == 'subcategory':
    ...
    try:
        async with state.proxy() as data:
            ...
            if data['item_type'] == 'product':
                ...
                # Создаем клавиатуру для возврата в категорию
                keyboard = InlineKeyboardMarkup()
                keyboard.add(
                    InlineKeyboardButton(
                        "🔙 Вернуться в категорию", 
                        callback_data=f"view_parent_{data['parent_id']}_0"
                    )
                )
                ...
                await message.answer(
                    ...
                    f"Описание: {'✓' if data.get('description') else '✗'}\n"
                    f"{files_summary}",
                    reply_markup=keyboard
                )
            else:
...
# === ОБРАБОТЧИКИ ДЛЯ РЕДАКТИРОВАНИЯ КНОПОК ГЛАВНОГО МЕНЮ ===

class AdminEditButtonStates:
    """Состояния для редактирования кнопок главного меню"""
    waiting_for_button_selection = "admin_edit_button_waiting_selection"
    waiting_for_new_name = "admin_edit_button_waiting_name"
...
async def editbutton_command_handler(message: types.Message):
    ...
    for button_key, button_name in buttons:
        keyboard.add(
            InlineKeyboardButton(
                button_name,
                callback_data=f"edit_button_{button_key}"
            )
        )
    
    keyboard.add(InlineKeyboardButton("🔙 Назад", callback_data="cancel_edit_button"))
    
    await message.answer(
        "✏️ <b>Редактирование кнопок главного меню</b>\n\n"
        "Выберите кнопку, которую хотите изменить:",
        parse_mode="HTML",
        reply_markup=keyboard
    )

@dp.callback_query_handler(lambda c: c.data.startswith("edit_button_"), state='*')
async def edit_button_selection_handler(callback: types.CallbackQuery, state: FSMContext):
    """Обработчик выбора кнопки для редактирования"""
    if not await is_user_admin(callback.from_user.id):
        await callback.answer("❌ У вас нет прав администратора", show_alert=True)
        return
    
    button_key = callback.data.split("_")[-1]
    
    # Получаем текущее название кнопки
    current_name = await db.get_button_name(button_key) or "Неизвестно"
    
    # Сохраняем выбранную кнопку в состоянии
    await state.update_data(editing_button_key=button_key)
    
    await state.set_state(AdminEditButtonStates.waiting_for_new_name)
    
    await callback.message.edit_text(
        f"✏️ <b>Редактирование кнопки</b>\n\n"
        f"📝 <b>Текущее название:</b> {current_name}\n\n"
        f"📝 Введите новое название кнопки:",
        parse_mode="HTML",
        reply_markup=InlineKeyboardMarkup().add(
            InlineKeyboardButton("🔙 Отмена", callback_data="cancel_edit_button")
        )
    )
    await callback.answer()

@dp.message_handler(state=AdminEditButtonStates.waiting_for_new_name, content_types=types.ContentTypes.TEXT)
async def edit_button_name_save(message: types.Message, state: FSMContext):
    """Обработчик сохранения нового названия кнопки"""
    try:
        data = await state.get_data()
        button_key = data.get('editing_button_key')
        
        if not button_key:
            await state.finish()
            await message.answer("❌ Ошибка: Не выбрана кнопка")
            return
        ...
        # Сохраняем новое название в базе данных
        await db.set_button_name(button_key, new_name)
        
        await message.answer(
            f"✅ Название кнопки успешно обновлено!\n\n"
        ...
        print(f"Error in edit_button_name_save: {e}")
        await message.answer("❌ Ошибка при обновлении названия")
        await state.finish()

@dp.callback_query_handler(lambda c: c.data == "cancel_edit_button", state='*')
async def cancel_edit_button_handler(callback: types.CallbackQuery, state: FSMContext):
    """Обработчик отмены редактирования кнопки"""
    await state.finish()
    await callback.message.edit_text("❌ Редактирование кнопки отменено.")
    await callback.answer()
